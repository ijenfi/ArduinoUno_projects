/*
Description: Get voltage value from the thermistor; get celsius value from voltage using a lookup table
*/
...
float getTemperature() {
	...
	float celsius = interpolate(table, adcReading, TABLE_SIZE);
	return celsius;
}

int getVoltage() {
	float voltage_value = analogRead(ANALOG_PIN);
	int voltage = (int)voltage_value;
	return voltage;
}